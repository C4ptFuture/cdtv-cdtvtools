...
int main (int argc, char **argv) 
{
    ...
    int i, j;
    BOOL updbootdelay = FALSE;
    BOOL resetbmstore = FALSE;
    ...
    for (i=1; i<argc; i++)
    {
        if (!strcmp(argv[i], "HDDBOOTDELAY")) {
            ...
            vblankfreq = 60;
        } else if (!strcmp(argv[i], "RESETBMSTORE")) {
            printf ("\t. Reset of bookmark memory requested.\n");
            resetbmstore = TRUE;
    ...
    if (resetbmstore) {
        /* write word length to $DC8000 */
        unsigned short *bookmarkmem = (unsigned short*)BOOKMARKMEM;
        *bookmarkmem = 0;
        printf ("\t. Done. Restart your system for bookmark reset to complete.\n");
    }
    else {
